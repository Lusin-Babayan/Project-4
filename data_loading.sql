...
select * from subscriptions order by subscriptions_id, user_id
where subscriptions_id <= 134
rollback;
...
from
(select distinct user_id, min(start_date) over(partition by user_id order by start_date) as start_date
from subscriptions)
as sub
